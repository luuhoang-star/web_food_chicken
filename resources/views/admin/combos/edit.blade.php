...
<div 
    ...
    x-data="{
        isDirty: false,
        imagePreview: '{{ str_starts_with($combo->image, 'http') ? $combo->image : asset($combo->image) }}',
        showUrlInput: false,
        imageUrl: '{{ $combo->image }}',
        name: '{{ addslashes($combo->name) }}',
        price: '{{ (int) $combo->price }}',
        originalPrice: '{{ $combo->original_price ? (int) $combo->original_price : '' }}',
        
        productsList: {{ json_encode($products->map(fn($p) => ['id' => $p->id, 'name' => $p->name])) }},
        
        items: {{ json_encode($combo->items->map(fn($i) => [
...
>
    ...
    <!-- 1. HEADER TINH GỌN (BREADCRUMB + TÊN COMBO + XOÁ) -->
    ...
    <form 
        id="comboEditForm"
        action="{{ route('admin.combos.update', $combo->id) }}" 
        method="POST" 
                ...
                <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs space-y-2.5">
                    ...
                    <label class="flex items-center justify-between p-2.5 rounded-xl bg-amber-50/60 border border-amber-200/80 cursor-pointer hover:bg-amber-50 transition-colors">
                        ...
                        <input 
                            ...
                            name="is_hot" 
                            value="1" 
                            {{ old('is_hot', $combo->is_hot) ? 'checked' : '' }} 
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <div class="space-y-1">
                                ...
                                <select name="tag" class="w-full px-3.5 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-xs font-bold text-gray-800 outline-none cursor-pointer">
                                    <option value="TIẾT KIỆM" {{ $combo->tag === 'TIẾT KIỆM' ? 'selected' : '' }}>TIẾT KIỆM</option>
                                    <option value="BEST SELLER" {{ $combo->tag === 'BEST SELLER' ? 'selected' : '' }}>BEST SELLER</option>
                                    <option value="HOT" {{ $combo->tag === 'HOT' ? 'selected' : '' }}>HOT</option>
                                    <option value="MÓN MỚI" {{ $combo->tag === 'MÓN MỚI' ? 'selected' : '' }}>MÓN MỚI</option>
                                </select>
                            </div>
                        </div>
    ...
    <div class="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-gray-200 shadow-2xl py-3 px-4 sm:px-8">
        <div class="max-w-5xl mx-auto flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2.5">
                ...
                <button 
                    type="button" 
                    @click="document.getElementById('comboEditForm').submit()"
