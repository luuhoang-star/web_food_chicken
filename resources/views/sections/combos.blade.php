    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
            @forelse($combos ?? [] as $index => $combo)
                @php
                    $isBestSeller = str_contains(strtoupper($combo->tag ?? ''), 'BÁN CHẠY') || $index === 1;
                @endphp
                <div class="{{ $isBestSeller ? 'bg-white rounded-3xl overflow-hidden border-2 border-red-600 shadow-xl flex flex-col justify-between transform lg:-translate-y-2 relative' : 'bg-[#FAF6F0] rounded-3xl overflow-hidden border border-gray-200 shadow-sm flex flex-col justify-between transition-all hover:shadow-lg' }}">
                    ...
                    <div>
                        <div class="relative aspect-[16/10] overflow-hidden bg-gray-200 cursor-pointer" @click="addToCartDirect({{ json_encode([
                            ...
                            @if($combo->tag)
                                <span class="absolute top-3 right-3 bg-amber-500 text-white text-[11px] font-extrabold px-2.5 py-1 rounded-md uppercase">{{ $combo->tag }}</span>
                            @endif
                        </div>
                    ...
                    <div class="p-6 pt-0 flex items-center justify-between border-t {{ $isBestSeller ? 'border-gray-100' : 'border-gray-200/60' }} mt-4">
                        ...
                        <button 
                            ...
                            type="button" 
                            class="px-6 py-2.5 rounded-full {{ $isBestSeller ? 'bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 text-white font-extrabold shadow-md red-glow' : 'bg-red-600 hover:bg-red-700 text-white font-bold shadow-xs' }} text-xs tracking-wide transition-all active:scale-95"
                        >
                            Chọn combo
                        </button>
                    </div>
